USE [CrudSql]
GO
/****** Object:  StoredProcedure [dbo].[sp_CRUDGenerate_SQL]    Script Date: 9/01/2023 08:50:20 ******/
SET ANSI_NULLS ON
GO
...
**               * Read				(Lee o presenta una sola registro o fila)
**               * ReadEager		(Lee o presenta una sola fila y/o columnas de tabla unidas por relación)
**               * Update			(Actualiza un simple registro o fila)
**               * UpdateMultiple	(Actualiza multiples registros o filas)
**               * Delete			(Borra un solo registro o fila)
**               * DeleteMultiple	(Borrar multiples registros o filas usando de forma segura T-SQL con parametros opcionales, 
**                                  o tecnicas de kitchen sink, Swiss army knife, catch-all queries)
**               * Search			(Busqueda con T-SQL dinámico seguro usando parámetros opcionales, kitchen sink, Swiss army knife, 
**                                  catch-all queries)
**               * Upsert			(Usa Try para actualizar el registro y si falla retorna el Insert )
**               * Indate			(try to insert the row first and fail back to update)
**
** Supports:     Ejecuta en versiones de SQL Server 2016, 2017, 2019, Azure SQL Server. 
**				 soporta JSON para cambiar con XML para 2014 y mas bajo.
**

** Notes:   sp_CRUDGenerate_SQL generará y regenerará automáticamente los procedimientos almacenados por usted. Si quieres personalizar
** 			uno de los procedimientos almacenados generados puede eliminar la sección de comentarios <auto-generated> y el almacenado
** 			el procedimiento no se sobrescribirá.
**
** 			Instalar y ejecutar sp_CRUDGenerate_SQL en la base de datos del usuario y no del maestro.
** 			Instalar y ejecutar sp_GenCrudSql en una base de datos de usuario y no en el maestro
** 			Tiempo de ejecución: depende de la complejidad de la estructura de su tabla.
**
** 			Bifurcar el repositorio para cambiar el estilo T-SQL (o formatear con una herramienta como Redgate SQL Prompt) y nombrar
** 			convenciones. Recuerde crear una solicitud de extracción si agregó algo interesante para que el resto de la comunidad
**          puede beneficiar.


**              Los nombres de la tablas deberian ser en (buena practica de escritura) PascalCase para un mejor nombramiento de alias de tablas .
**
**              Use FOREIGN KEY REFERENCES between tables for ReadEager and Search to recurse over related tables.
**
**              There are paramaters you can set in sp_CRUDGenerate_SQL to customize for your column naming convention.
**
...
CREATE PROCEDURE [dbo].[sp_CRUDGenerate_SQL] (
...
AS
    BEGIN
        ...
        **********************************************************************************************************************/
        SET @Version = '4.00.01';
        SET @VersionDate = '20230201';
        SET @ScriptVersionName = N'sp_CRUDGenerate_SQL v' + @Version + N' - ' + DATENAME(MONTH, @VersionDate) + N' ' + RIGHT('0' + DATENAME(DAY, @VersionDate), 2) + N', ' + DATENAME(YEAR, @VersionDate);
...
** MIT License
** 
** Copyright (c) ' + CAST(YEAR(GETUTCDATE()) AS nvarchar(MAX)) + N' HV.FullStackDv
** 
*/';
        SET @AutoGeneratedCommentString = N'/* <auto-generated>
    This stored procedure was generated from the stored procedure named ' + CAST(ISNULL(OBJECT_SCHEMA_NAME(@@PROCID) + '.' + OBJECT_NAME(@@PROCID), 'dbo.sp_CRUDGenerate_SQL') AS nvarchar(MAX)) + N'
    NOTES: You can remove this comment section to keep this stored procedure from being overwritten.
        ...
        **********************************************************************************************************************/
        ...
        SELECT
           ...
           ,ProcedureName       = CAST(P.ProcedureType + T.name AS sysname)
           ...
           ,IsExistsFlag        = CASE WHEN EXISTS (
                                                ...
                                                WHERE
                                                    S2.name = S.name
                                                AND O2.name = CAST(P.ProcedureType + T.name AS sysname)
                                            )
                                           THEN 1
                                      ELSE 0
                                  END
        ...
        FROM
            ...
            CROSS JOIN (
                SELECT
                    ProcedureType =  'spu_ins_o_'  -- Create Ome spu_ins_o_
                WHERE
                    @GenerateCreate = 1
                UNION ALL
                SELECT
                    ProcedureType =  'spu_ins_m_'  -- Create Multiple spu_ins_m_
                WHERE
                    @GenerateCreateMultiple = 1
                UNION ALL
                SELECT
                    ProcedureType =  'spu_sel_o_' --'Read' spu_sel_o_
                WHERE
                    @GenerateRead = 1
                UNION ALL
                SELECT
                    ProcedureType =  'spu_sel_x_' -- 'ReadEager' spu_sel_x_
                WHERE
                    @GenerateReadEager = 1
                UNION ALL
                SELECT
                    ProcedureType =   'spu_upd_o_' -- 'Update' spu_upd_o_
                WHERE
                    @GenerateUpdate = 1
                UNION ALL
                SELECT
                    ProcedureType =   'spu_upd_m_' -- 'UpdateMultiple' spu_upd_m_
                WHERE
                    @GenerateUpdateMultiple = 1
                UNION ALL
                SELECT
                    ProcedureType =   'spu_upsert_'    -- spu_upsert_
                WHERE
                    @GenerateUpsert = 1
                UNION ALL
                SELECT
                    ProcedureType =   'spu_indate_'    -- spu_indate_
                WHERE
                    @GenerateIndate = 1
                UNION ALL
                SELECT
                    ProcedureType =   'spu_del_o_' -- 'Delete' spu_del_o_
                WHERE
                    @GenerateDelete = 1
                UNION ALL
                SELECT
                    ProcedureType =   'spu_del_m_' -- 'DeleteMultiple' spu_del_m_
                WHERE
                    @GenerateDeleteMultiple = 1
                UNION ALL
                SELECT
                    ProcedureType =   'spu_search_x_' -- Search spu_search_x_
                WHERE
                    @GenerateSearch = 1
            )                                       AS P
        WHERE
        ...
        SELECT
           ...
           ,TableName           = V.name
           ,TableDescription    = ISNULL(CAST(EP.value AS nvarchar(MAX)), N'')
           ,ProcedureName       = CAST(P.ProcedureType + V.name   AS sysname)
           ,ProcedureType       = P.ProcedureType
           ,IsExistsFlag        = CASE WHEN EXISTS (
                                                SELECT
                                                    *
                                                ...
                                                WHERE
                                                    S2.name = S.name
                                                AND O2.name = CAST(P.ProcedureType + V.name  AS sysname)
                                            )
                                           THEN 1
                                      ELSE 0
                                  END
        ...
        FROM
            ...
            CROSS JOIN (
                SELECT
					...
					ProcedureType = 'spu_ins_o_'
                WHERE
                    @GenerateCreate = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'CreateMultiple'
					ProcedureType = 'spu_ins_m_'
                WHERE
                    @GenerateCreateMultiple = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'Read'
					ProcedureType = 'spu_sel_o_'
                WHERE
                    @GenerateRead = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'ReadEager'
					ProcedureType = 'spu_sel_x_'
                WHERE
                    @GenerateReadEager = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'Update'
					ProcedureType = 'spu_upd_o_'
                WHERE
                    @GenerateUpdate = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'UpdateMultiple'
					ProcedureType = 'spu_upd_m_'
                WHERE
                    @GenerateUpdateMultiple = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'Upsert'
					ProcedureType = 'spu_upsert_'
                WHERE
                    @GenerateUpsert = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'Indate'
					ProcedureType = 'spu_indate_'
                WHERE
                    @GenerateIndate = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'Delete'
					ProcedureType = 'spu_del_o_'
                WHERE
                    @GenerateDelete = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'DeleteMultiple'
					ProcedureType = 'spu_del_m_'
                WHERE
                    @GenerateDeleteMultiple = 1
                UNION ALL
                SELECT
                    -- ProcedureType = 'Search'
					ProcedureType =   'spu_search_x_' -- Search
                WHERE
                    @GenerateSearch = 1
            )                                       AS P
        WHERE
        ...
        WHILE EXISTS (SELECT * FROM #StoreProcedureList AS TL WHERE TL.IsProcessedFlag = 0)
            BEGIN
                ...
                **********************************************************************************************************************/
				...
				IF @ProcedureType IN (N'spu_ins_o_', N'spu_ins_m_', N'spu_upd_o_', N'spu_upd_m_', N'spu_upsert_', N'spu_indate_')
                    BEGIN
                        SELECT
				...
				IF @ProcedureType IN (N'spu_ins_o_', N'spu_ins_m_', N'spu_upd_o_', N'spu_upsert_', N'spu_indate_')
                    BEGIN
                        SELECT
				...
				IF @ProcedureType IN (N'spu_upd_m_')
                    BEGIN
                        SELECT
                            @TemporaryTableStringColumnType = @TemporaryTableStringColumnType + @NewLineString + N'/*[INDENT SPACES]*/,' + QUOTENAME(CL.ColumnName) + CAST(N' ' AS nvarchar(MAX)) + REPLACE(CL.TypeName, 'rowversion', 'nvarchar(20)') + CL.TypeLength + N' NULL' + CASE WHEN LEN(CL.ColumnDescription) > 0
                                                                                                                                                                                                                                                                                           THEN CAST(N' /* ' AS nvarchar(MAX)) + CL.ColumnDescription + CAST(N' */' AS nvarchar(MAX))
				...
				IF @ProcedureType IN (N'spu_upd_o_',  N'spu_upsert_', N'spu_indate_')
				...
				IF @ProcedureType IN ('spu_ins_o_', 'spu_upd_m_', 'spu_ins_m_')
...
AS
    ...
    END;'               ;
                ...
                **********************************************************************************************************************/
				...
				IF @ProcedureType = N'spu_upsert_' 
...
'                       ;
    ...
    END;'               ;
                ...
                **********************************************************************************************************************/
				...
				IF @ProcedureType = N'spu_indate_'
...
'' AS [processing-instruction(output)] FOR XML PATH('''')
    ...
    END;'               ;
                ...
                **********************************************************************************************************************/
				...
				IF @ProcedureType = N'spu_search_x_'
                    BEGIN
                        /**********************************************************************************************************************
...
If you execute dbo.sp_CRUDGenerate_SQL with @GenerateStoredProcedures = 1 it will create the stored procedures automatically.
Copy just the T-SQL below this block comment into a new query window to execute. */
